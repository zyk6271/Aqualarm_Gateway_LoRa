...
#define MCU_VER "1.0.13"                                 //用户的软件版本,用于MCU固件升级,MCU升级版本需修改
...
#define MCU_PACKAGE_SIZE                   0x02      //1024byte
#define SUB_PACKAGE_SIZE                   0x03      //128byte
#endif
/******************************************************************************
                         3:定义收发缓存:
                    用户根据实际情况定义收发缓存的大小
...
//水浸报警(只上报)
//备注:默认状态为normal
#define ENDUNIT_DPID_WATERSENSOR_STATE 1
//信号强度(只上报)
//备注:
#define ENDUNIT_DPID_RSSI 101
//剩余电量(只上报)
//备注:0表示100%，1表示40%，2表示10%，3表示80，4表示60%
#define ENDUNIT_DPID_BATTERY_STATES 102
//终端 / 新型终端控制(只上报)
//备注:
#define ENDUNIT_DPID_CONTROL_STATE 103
//测水线掉落(只上报)
//备注:1为掉落，0为正常
#define ENDUNIT_DPID_LINE_STATE 104
//绑定主控⁺ ID(只上报)
//备注:
#define ENDUNIT_DPID_BIND_ID 107
...
//high信号高
//
#define DOORUNIT_DPID_RSSI 101
//剩余电量(只上报)
//备注:0表示100%，1表示40%，2表示10%，3表示80，4表示60%
#define DOORUNIT_DPID_BATTERY 102
//门控控制(只上报)
//备注:开阀方式2：门控中的开关执行开关阀
#define DOORUNIT_DPID_CONTROL_STATE 104
//延时关阀(只上报)
//备注:开阀方式3：延时关执行器
#define DOORUNIT_DPID_DELAY_STATE 105
//绑定主控⁺ ID(只上报)
//备注:
#define DOORUNIT_DPID_BIND_ID 107
...
//阀门状态(只上报)
//备注:设备自身开关阀上报(0关1开)
#define MAINUNIT_DPID_DEVICE_STATE 101
//阀门1 自检失败(只上报)
//备注:主控1号阀门自检失败结果上报到报警记录同时关联图标逻辑（True代表报警关联报警记录，False为恢复正常不关联）
#define MAINUNIT_DPID_VALVE1_CHECK_FAIL 102
//水浸报警(只上报)
//备注:主控水警上报（0普1警）
#define MAINUNIT_DPID_DEVICE_ALARM 103
//阀门控制(可下发可上报)
//备注:开阀方式：APP对主控执行开关阀(0关1开)
#define MAINUNIT_DPID_CONTROL_STATE 104
//信号强度(只上报)
//备注:<-94 一格信号：0
//-94 ~ -78 两格信号：1
//>-78 三格信号：2
#define MAINUNIT_DPID_SIGN_STATE 105
//延时关阀(只上报)
//备注:延时开关执行开关阀
#define MAINUNIT_DPID_DELAY_STATE 106
//低温状态(只上报)
//备注:0:温度正常
//1.温度过低
#define MAINUNIT_DPID_TEMP_STATE 107
//门控 ID(只上报)
//备注:
#define MAINUNIT_DPID_DOOR_ID 108
//测水线掉落(只上报)
//备注:0普1警
#define MAINUNIT_DPID_LINE_STATE 109
//延时开关倒计时(可下发可上报)
//备注:0:倒计时1h
//1：倒计时2h
//2：倒计时3h
//3：倒计时4h
#define MAINUNIT_DPID_DELAY_TIME 110
//设备状态(可下发可上报)
//备注:normal == true
#define MAINUNIT_DPID_NORMAL_STATA 111
//主控⁺ ID(只上报)
//备注:
#define MAINUNIT_DPID_SELF_ID 112
//阀门2 自检失败(只上报)
//备注:主控2号阀门自检失败结果上报到报警记录同时关联图标逻辑（True代表报警关联报警记录，False为恢复正常不关联）
#define MAINUNIT_DPID_VALVE2_CHECK_FAIL 113
//阀门1 自检成功(只上报)
//备注:主控1号阀门自检成功结果上报到操作记录（True代表成功关联操作记录，False情况不存在不关联）
#define MAINUNIT_DPID_VALVE1_CHECK_SUCCESS 114
//阀门2 自检成功(只上报)
//备注:主控2号阀门自检成功结果上报到操作记录（True代表成功关联操作记录，False情况不存在不关联）
#define MAINUNIT_DPID_VALVE2_CHECK_SUCCESS 115
...
//网关ID(只上报)
//备注:
#define GATEWAY_DPID_SELF_ID 101
