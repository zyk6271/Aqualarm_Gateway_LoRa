...
{
    {MAINUNIT_DPID_CONTROL_STATE, DP_TYPE_BOOL},
};
...
static unsigned char dp_download_control_state_handle(const unsigned char value[], unsigned short length, unsigned char *sub_id_buf, unsigned char sub_id_len)
{
    ...
    unsigned char control_state;
     ...
     uint32_t device_id = atol(sub_id_buf);

    control_state = mcu_get_dp_download_bool(value,length);

    radio_mainunit_remote_control(device_id,control_state);

    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(MAINUNIT_DPID_CONTROL_STATE,control_state, sub_id_buf, sub_id_len);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length,unsigned char* sub_id_buf,unsigned char sub_id_len)
{
    ...
    ****************************************************************/
    ...
    switch(dpid)
    {
        case MAINUNIT_DPID_CONTROL_STATE:
...
//    devkey = item->valuestring;
    ...
    radio_mainunit_remote_delete(atol(sub_id));
    wifi_device_delete(atol(sub_id));
    aq_bind_delete(atol(sub_id));
    aq_device_delete(atol(sub_id));
...
unsigned char subdev_firm_update_handle(unsigned char *sub_id_buf,const unsigned char value[],unsigned long position,unsigned short length)
{
    ...
    if(length == 0)
    {
        lora_ota_end(atol(sub_id_buf));
        //固件数据发送完成
    }
    else
    {
        lora_ota_receive(atol(sub_id_buf),value,position,length);
        //固件数据处理
    }
...
void qur_subdev_ver_get(unsigned char* data_buf,unsigned short data_len)
{
    ...
    aqualarm_device_t *device = aq_device_find(atol(sub_id));
    if(device == RT_NULL)
    {
        return;
    }
    ...
    qur_subdev_ver_report(sub_id,ver_str);
